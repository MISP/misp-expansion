//This file contain all of the string used in the code

const EMPTY = "";
const RETOURCHARIOT = "\n";
const INFO = "Info";

/////////////////I18N SECTION/////////////////
//if you modify one of this value, you also have to modify them in all of the message.json file

const WARNING = "warning";
const RESFAIL = "requestFail";
const WRNMESSAGE = "warningMessage";
const INFOTITLE = "infoTitle";
const NOCRED = "noCredentials";
const ERASEDONE = "eraseDone";
const EMPTYCHAMP = "emptyChamp";
const IDSAV = "idSaved";
const ERASECONFIRM = "eraseConfirm";
const EVENTNAME = "event";
const NBOFELEMENT = "nbOfElement";
const NBOFOBJECT = "nbOfObject";
const NBOFEVENT = "nbOfEvent";

///////////////////HTML ID//////////////////////
//if you modify one of this value, you also have to modify them in all of the message.json file and the html file if this extension
//and the button.onclick name too

const SAVEBUTTON = "saveButton";//button
const LOGTEXT = "loginTexte";
const PWDTEXT = "pwdTexte";
const ERASE = "erase";//button
const NORI = "norInput";
const LOGINPUT = "logInput";
const PWDINPUT = "pwdInput";
const VIEW = "view";
const INSTANCE = "instance";
const NBOFRES = "numberOfRes";
const CAP = "caption";
const TABLE = "InstanceTable";
const OPTREQUEST = "optionRequest";
const EMPTYRESULT = "emptyResult";

export {EMPTY, WARNING, RESFAIL, WRNMESSAGE, INFOTITLE, NOCRED, ERASEDONE, EMPTYCHAMP, IDSAV, CAP, TABLE, EVENTNAME, OPTREQUEST, INFO, NBOFEVENT,
 SAVEBUTTON, LOGTEXT, PWDTEXT, ERASE, NORI, LOGINPUT, PWDINPUT, VIEW, INSTANCE, NBOFRES, ERASECONFIRM, RETOURCHARIOT, NBOFELEMENT, NBOFOBJECT, EMPTYRESULT};